...
class MojRSS(Screen):
    ...
    def Reload(self):
        ...
        if os.path.exists('/var/ddRSS/feeds'):
            try:
                ...
                self.session.open(
                    MessageBox,
                    f"Errore nel ricaricamento dei feed: {e}",
    ...
    def izlaz(self):
        with open('/var/ddRSS/feeds', 'w') as fp:
            # Usa zip per combinare le liste 'ime' e 'put' e scrivere
            # direttamente
            ...
            try:
                ...
                with open('/tmp/lirss', 'w') as fp1:
                    if contenuto_completo:
                        ...
                        if n2 > -1:
                            contenuto_completo = skrati(
                                n2 + 6, contenuto_completo)
                        ...
                        if n0 > -1:
                            ...
                            fp1.write(
                                f'0<DD>{codifica}<DD>{titolo}<DD>nessuno\n')
                        ...
                        for elemento in elementi:
                            ...
                            data = uzmitekst(
                                n1 + 9, n2, elemento) if n1 > -1 else 'nessuna data disponibile'
                            date.append(data)
...
class PregledRSS(Screen):
    ...
    def stvorilistu(self):
        ...
        if os.path.exists('/tmp/lirss'):
            ...
            with open('/tmp/lirss', 'r', encoding='utf-8') as fp:
                for line in fp.read().split('\n'):
                    if len(line) != 0:
                        razbi = line.split('<DD>')
                        if prvi == 1:
