...
exec > >(tee -a /tmp/DDRSSReader_debug.txt) 2>&1
set -x
...
wget --no-check-certificate 'https://github.com/Belfagor2005/DDRSSReader/archive/refs/heads/main.tar.gz'
tar -xzf main.tar.gz
cp -r 'DDRSSReader-main/usr' '/'
set +e
cd
sleep 2
...
IMAGE VERSION: $distro_version" >> /tmp/DDRSSReader_debug.txt
sleep 5
killall -9 enigma2
exit 0